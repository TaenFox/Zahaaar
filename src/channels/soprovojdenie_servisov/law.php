            ...
            case "message":
                switch(SLCK_EVENT_SUBTYPE)
                {
                    case "bot_message":
                        ...
                        */
                            sleep(3); //пауза, потому что HALP при моментальной установке делает два тикета
                            logger("Ссылка на тред - ".SLCK_EVENT_THREAD_TS);
                            if (true/*is_null(SLCK_EVENT_THREAD_TS) - это не работает, так как сообщения топик создаётся сразу в треде*/)
                            {
